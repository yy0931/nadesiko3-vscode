...
class NakoGen {
  ...
  reset() {
    ...
    // 初期化メソッド以外の関数を削除
    const uf = {}
    for (const key in this.used_func) { if (key.match(/^!.+:初期化$/)) { uf[key] = this.used_func[key] } }
  ...
  getDefFuncCode(isTest) {
    ...
    if (nakoFuncCode !== '') { code += '__v0.line=0;// なでしこの関数定義\n' + nakoFuncCode }

    // プラグインの初期化関数を実行する
    let pluginCode = ''
    for (const name in this.__module) {
      const initkey = `!${name}:初期化`
      if (this.__varslist[0][initkey]) { pluginCode += `__v0["!${name}:初期化"](__self);\n` } // セミコロンがないとエラーになったので注意

    }
    if (pluginCode !== '') { code += '__v0.line=0;// プラグインの初期化\n' + pluginCode }
  ...
  registerFunction(ast) {
    if (ast.type !== 'block') { throw new NakoGenError('構文解析に失敗しています。構文は必ずblockが先頭になります') }
  ...
  findVar(name) {
    // __vars ? (ローカル変数)
    if (this.__vars[name] !== undefined) { return { i: this.__varslist.length - 1, 'name': name, isTop: true } }

    // __varslist ?
    for (let i = this.__varslist.length - 2; i >= 0; i--) {
      const vlist = this.__varslist[i]
      if (!vlist) { continue }
      if (vlist[name] !== undefined) { return { 'i': i, 'name': name, isTop: false } }
  ...
  genVar(name, line) {
    ...
    const lno = line
    if (res === null) { return `__vars["${name}"]/*?:${lno}*/` }
  ...
  convReturn(node) {
    // 関数の中であれば利用可能
    if (typeof (this.__vars['!関数']) === 'undefined') { throw new NakoGenError('『戻る』がありますが、関数定義内のみで使用可能です。', node.line) }
  ...
  convDefFuncCommon(node, name) {
    ...
    if (name) { this.__varslist[1][name] = code }
  ...
  convForeach(node) {
    let target
    if (node.target === null) { target = this.sore }
    else { target = this._convGen(node.target) }
  ...
  convFuncGetArgsCalcType(funcName, func, node) {
    ...
    for (let i = 0; i < node.args.length; i++) {
      ...
      if (i === 0 && arg === null) {
        args.push(this.sore)
        opts['sore'] = true
      } else { args.push(this._convGen(arg)) }
  ...
  convLet(node) {
    ...
    if (isTop) { code = `__vars["${name}"]=${value};` }
    else { code = `__varslist[${res.i}]["${name}"]=${value};` }
