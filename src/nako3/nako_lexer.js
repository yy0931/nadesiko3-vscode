...
import { opPriority } from './nako_parser_const'

// 予約語句
// (memo)「回」「間」「繰返」「反復」「抜」「続」「戻」「代入」などは replaceWord で word から変換
import reservedWords from './nako_reserved_words'

// 助詞の一覧
import josi from './nako_josi_list'
const josiRE = josi.josiRE

// 字句解析ルールの一覧
import lexRules from './nako_lex_rules'
...
class NakoLexer {
  ...
  preDefineFunc(tokens) {
    ...
    const readArgs = () => {
      ...
      for (const arg of args) {
        if (!already[arg.value]) {
          ...
          if (arg.funcPointer) { funcPointers.push(arg.value) }
    ...
    while (i < tokens.length) {
      ...
      // 関数名
      if (tokens[i] && tokens[i].type === 'word') { funcName = tokens[i++].value }
      ...
      for (const rule of rules) {
        ...
        if (rule.cbParser) {
          ...
          if (rule.name === 'string_ex') {
            ...
            if (list.length >= 1 && list.length % 2 === 0) { throw new Error('字句解析エラー(' + (line + 1) + '): 展開あり文字列で値の埋め込み{...}が対応していません。') }
