 ...
 */
import { opPriority, keizokuJosi } from './nako_parser_const'
import { NakoParserBase, NakoSyntaxError } from './nako_parser_base'
const operatorList = []
...
class NakoParser extends NakoParserBase {
  ...
  yDef(type) {
    ...
    try {
      if (multiline) {
        ...
        else { throw new NakoSyntaxError('『ここまで』がありません。関数定義の末尾に必要です。', def.line, this.filename) }
        this.loadStack()
      } else {
        this.saveStack()
  ...
  yCallFunc() {
    ...
    if (!f || typeof f['josi'] === 'undefined') { throw new NakoSyntaxError('関数の定義でエラー。', t.line) }
    ...
    // 1つだけなら、変数「それ」で補完される
    if (nullCount >= 2 && (0 < valueCount || t.josi === '' || keizokuJosi.indexOf(t.josi) >= 0)) { throw new NakoSyntaxError(`関数『${t.value}』の引数が不足しています。`, t.line, this.filename) }
  ...
  yValueKakko() {
    ...
    if (!this.check(')')) { throw new NakoSyntaxError('(...)の解析エラー。' + this.nodeToStr(v) + 'の近く', t.line, this.filename) }
  ...
  yValue() {
    ...
    if (this.check2([['func', 'word'], '(']) && this.peek().josi === '') {
      ...
      else { throw new NakoSyntaxError('C風関数呼び出しのエラー', f.line, this.filename) }
  ...
  yJSONObjectValue() {
    ...
    while (!this.isEOF()) {
      ...
      } else { throw new NakoSyntaxError('辞書オブジェクトの宣言で末尾の『}』がありません。', firstToken.line, this.filename) }
