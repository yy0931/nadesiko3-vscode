...
export default {
  '初期化': {
    ...
    fn: function (sys) {
      ...
      sys.__getBokanPath = () => {
        ...
        if (cmd.indexOf('cnako3') < 0) { nakofile = process.argv[1] }
        else { nakofile = process.argv[2] }
  ...
  '保存': { // @データSをファイルFヘ書き込む // @ほぞん
    ...
    fn: function (s, f) {
      // Buffer?
      if (s instanceof String) { fs.writeFileSync(f, s, 'utf-8') }
  ...
  'POSTフォーム送信時': { // @AjaxでURLにPARAMSをフォームとしてPOST送信し『対象』にデータを設定 // @POSTふぉーむそうしんしたとき
    ...
    fn: function (callback, url, params, sys) {
      ...
      for (let key in params) { fd.set(key, params[key]) }
